...
## 16. 真实项目拆解：把一个 Agent CLI 讲成工程闭环

到这里不要再只背“应该有 Provider、RAG、MCP、sandbox”。拿一个能运行的项目，把**文件边界、一次请求的数据流、已经验证的能力和还没补齐的缺口**讲出来，面试官才会相信你真的写过 Agent。

下面用一个 TypeScript 终端编码 Agent（下文称 `agent-cli`）作案例。它不是用来炫耀“我造了一个 Copilot”，而是用一个足够小的项目证明：你能把模型 API 封装成一个可演进的 Agent runtime。
...
用户输入
   ...
   ▼
src/cli.ts
   │  读取工作区、组装 system message、注册工具、打印用量
   ▼
src/config.ts ──校验──> Azure 配置（不输出 API key）
   │
   ▼
src/provider.ts: ModelProvider
   │  Azure OpenAI 协议、消息序列化、tool_calls 反序列化、usage
   ▼
src/agent.ts: runAgent
   │  最大 10 轮；模型决策 → policy check → 工具执行 → tool 回填
   ├────────────────────────────────────────────────────┐
   ▼                                                    ▼
src/sandbox.ts                                     src/tools/*
策略边界：只读/命令/域名限制                    文件、搜索、typecheck、diff、网络
   │                                                    │
   ├───────────────┬──────────────────────┬─────────────┘
   ▼               ▼                      ▼
session        MCP client               AST-RAG
JSON 历史      JSON-RPC stdio           TypeScript export → embedding → SQLite cache
```

这张图最关键的不是模块数量，而是**依赖方向**：`agent.ts` 只依赖 `ModelProvider` 和 `Tool` 接口，不依赖 Azure HTTP 格式；`provider.ts` 才负责 API key、URL、消息格式和供应商返回值。以后换模型供应商时，不应把改动扩散进 Agent loop。

> **Pi 类比**：这对应 Pi 把模型能力收在 `packages/ai` 边界之下，Agent runtime 只面对稳定抽象；CLI/TUI 只负责输入、展示和会话驱动。面试时说“我把 provider protocol 留在适配层，不让 runtime 知道 Azure/OpenAI 的 payload 细节”，比说“我用了某某 SDK”强得多。

### 16.2 一次“修 TypeScript 错误”的运行时数据流

假设用户输入：“运行类型检查，定位错误并说明可能的修复方向。”

1. `cli.ts` 构造系统消息，其中包含当前工作目录、文件树、符号索引和可用工具。
2. `runAgent()` 将历史消息与工具 schema 交给 `ModelProvider.complete()`；Azure 适配层把它序列化为 Chat Completions 请求。
3. 模型返回 `tool_calls`，例如 `run_typecheck`；这仍是**意图**，并没有在模型侧执行命令。
4. runtime 先调用 `validateToolCall()`。若处于只读模式、命令不在白名单、URL 不在允许域名内，就把拒绝原因作为 tool result 回填，而不是执行。
5. 通过校验的调用交给 `Tool.execute()`。`run_typecheck` 会执行既定命令，并把编译输出解析成 `file / line / column / TS code / severity` 的结构化错误。
6. 所有同轮 tool call 用 `Promise.all` 并行执行，但按模型给出的顺序回填 `role: "tool"` 消息；模型据此继续调用工具或给最终回答。
7. CLI 汇总 prompt、completion 和 cached token，并显示实际成本及 KV cache 节省。

这比“Agent 会自动修 bug”准确得多。当前项目已经能**发现并结构化错误**，但“错误 → 检索相关代码 → 生成补丁 → typecheck → 重试”的完整自修复闭环尚未接入 runtime。主动说明这个边界，反而是加分项。

### 16.3 把源码文件变成你的面试证据

| 你要讲的能力 | 对应源码 | 不要夸大的地方 |
|---|---|---|
| Provider 抽象与用量统计 | `src/provider.ts`、`src/cli.ts` | 目前实现的是 Azure Chat Completions 适配，不等于已支持多供应商切换。 |
| ReAct 式 tool loop | `src/agent.ts` | 有默认 10 轮上限；`tokenBudget`、上下文压缩是 runtime 可选能力，当前 CLI 入口并未默认开启。 |
| 工具安全边界 | `src/sandbox.ts`、`src/tools/*` | `cli.ts` 的学习默认配置允许写入和命令执行；生产环境必须传入只读/目录/命令/域名白名单。 |
| 语义代码检索 | `src/rag.ts`、`src/tools/semanticSearch.ts` | 用 TypeScript AST 按 export 分块、SQLite 缓存、余弦相似度 O(N) 扫描；适合小中型工作区，不是十万 chunk 的 HNSW 服务。 |
| MCP 适配 | `src/mcp.ts` | 已实现 stdio JSON-RPC 握手、tools/list、tools/call 到内部 Tool 的包装；当前 CLI 入口尚未把 `--mcp` 接线。 |
| 错误恢复基础 | `src/tools/errorRecovery.ts` | 已有 typecheck 解析和错误上下文；还缺自动规划、补丁审批与重试闭环。 |
| 会话与成本 | `src/session.ts`、`src/cli.ts` | 当前 session 保存对话历史，尚不是带权限、加密、检索和生命周期治理的生产记忆系统。 |

这张表的作用是让你始终说清楚“**已实现**”“**设计为可扩展**”“**下一步要做**”三个层次。面试最怕把 demo 的能力说成生产 SLA；诚实的边界感本身就是工程能力。

### 16.4 三分钟项目陈述：可直接练习

> 我实现了一个 TypeScript 终端编码 Agent，目标不是让模型直接碰文件系统，而是把模型的工具调用意图放进受控 runtime。入口层只负责组装工作区上下文和交互；模型协议收在 `ModelProvider` 接口后，Azure 的鉴权、消息序列化、tool call 解析和 token usage 都留在 provider adapter。
>
> runtime 用有最大轮数的 Agent loop 承接模型决策。模型返回 tool call 后，我先经过 sandbox policy，再分发给统一的 Tool 接口执行，并把结果按 `role: tool` 回填给模型。这样模型永远没有直接的 shell 或文件权限。对代码理解，我没有按固定行数截断，而是用 TypeScript AST 按顶层 export 分块，嵌入后缓存进 SQLite；因此语义搜索能返回完整函数或类型声明。
>
> 我还把类型检查做成了工具，编译错误会被解析成文件、行列、错误码和摘要，供后续检索与修复使用。当前我会明确它的边界：错误检测已完成，但自动规划和修复重试尚未闭环；默认 CLI 的 sandbox 也不能直接当生产策略。下一步我会先给写入和命令工具加显式审批与最小权限，再把错误恢复做成可回放的 golden task。

### 16.5 高频追问：从真实取舍展开

**Q：为什么不让 `agent.ts` 直接调用 Azure SDK？**
答：因为 runtime 需要稳定地表达“发消息、收文本、收 tool call、收 usage”，不需要知道 endpoint、API version 或供应商 payload。把协议细节放进 provider adapter，测试也可以注入 fake provider，避免 Agent loop 和云供应商强耦合。

**Q：为什么同一轮工具调用要并行？有什么风险？**
答：独立的只读调用，例如读两个文件、查两个索引，可以并行降低 wall-clock latency；但写文件、修改同一资源、先读后写这类有依赖或副作用的调用不能盲目 `Promise.all`。生产版应根据 tool metadata 建 DAG、串行化冲突资源，并为写操作加审批和幂等键。

**Q：这个 sandbox 已经安全吗？**
答：它提供了正确的执行前 policy hook：只读模式、命令白名单、搜索根目录和 URL 域名白名单。但当前学习 CLI 默认 `readOnly: false`，且未设置 `allowedCommands`，因此它证明了边界的位置，不证明默认配置可安全地服务不可信请求。生产化应默认拒绝、按租户配置 capability，并记录审计事件。

**Q：你的 RAG 为什么不用向量数据库服务？**
答：本地 CLI 的约束是启动快、依赖少、工作区规模有限。AST 分块避免了正则切坏 TypeScript 声明，SQLite cache 避免每次重建 embedding，O(N) cosine scan 在小中型代码库足够直接。规模扩大后再替换检索后端为 ANN/HNSW，而不是过早引入分布式系统。

### 16.6 面试前的“真实项目”自检

- 能从终端演示一次 `read_file → tool result → final answer`，并解释每一跳是谁执行的。
- 能指出一个**已完成**能力：AST-RAG、typecheck 错误结构化、KV cache 用量统计。
- 能指出一个**未完成**能力：任务规划、完整 error recovery、严格默认 sandbox、MCP CLI 接线。
- 能说出下一步不是“再加更多工具”，而是“把写入/命令能力默认收紧，给修复闭环加审批、回归和重试上限”。

这就是把“我做过 Agent demo”升级为“我理解 Agent 工程边界”的最短路径。
...
- 用真实 Agent CLI 讲项目时，必须区分“已实现”“可扩展”和“尚未闭环”；能讲清 provider 边界、运行时 policy、工具回填、AST-RAG 与明确缺口，比堆框架名更有说服力。
